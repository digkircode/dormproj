...
export class IndividualsSyncService {
  ...
  async syncOne(uid: string): Promise<{ fetchedCount: number; added: number; updated: number; records: unknown[] }> {
    ...
    return { fetchedCount: records.length, added, updated, records };
  }
