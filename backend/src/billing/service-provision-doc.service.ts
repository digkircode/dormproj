...
export class ServiceProvisionDocService {
  ...
  async run(targetMonth: Date = addMonths(new Date(), -1)): Promise<{ pushed: number; succeeded: number; failed: number; skipped: boolean }> {
    if (!this.provider.isServiceProvisionConfigured()) {
      this.logger.warn('1С Бухгалтерия (оказание услуг) не настроена — пропуск');
    ...
    if (lines.length === 0) {
      this.logger.log(`Оказание услуг за ${monthStart.toISOString().slice(0, 7)}: нет ни одного договора с начислением за этот месяц и уже известной парой ContractorUID/ContractUID — нечего отправлять`);
      return { pushed: 0, succeeded: 0, failed: 0, skipped: true };
    }
    ...
    const rowByType = new Map<ServiceProvisionType, { id: number; accounting1cDocumentUid: string | null }>();
    for (const spec of specs) {
      ...
      // upsert ДО отправки — чтобы взять уже сохранённый accounting1cDocumentUid (если это
      // повтор после сбоя) и передать его в запросе, тогда 1С ОБНОВИТ существующий
      // документ, а не создаст дубль (см. промпт проекта).
    ...
    if (items.length === 0) {
      this.logger.log(`Оказание услуг за ${monthStart.toISOString().slice(0, 7)}: суммы по найму и коммуналке нулевые у всех известных договоров — нечего отправлять`);
      return { pushed: 0, succeeded: 0, failed: 0, skipped: true };
    }
    ...
    let succeeded = 0;
    let failed = 0;
    for (const row of rowByType.values()) {
      const result = results.find((r) => r.SiteDocumentID === row.id);
      if (!result) continue;
      if (result.FinalStatus) {
        ...
        await this.prisma.serviceProvisionDocument.update({
          ...
          data: {
            ...
            accounting1cDocumentUid: result.DocumentUID ?? row.accounting1cDocumentUid,
    ...
    this.logger.log(
      `Оказание услуг за ${monthStart.toISOString().slice(0, 7)}: отправлено документов ${items.length}, успешно ${succeeded}, ошибок ${failed}`,
    );
