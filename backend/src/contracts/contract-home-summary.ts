...
export async function buildContractHomeSummary(
  ...
  const nextAccrual = accrualBalances.find((a) => a.balance.greaterThan(0)) ?? null;
  ...
  return {
    ...
    nextAccrual: nextAccrual ? { dueDate: nextAccrual.dueDate, balance: Number(nextAccrual.balance) } : null,
  };
}
